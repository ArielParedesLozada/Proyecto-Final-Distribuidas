...
export interface ConsumptionComparisonItem {
  routeId: string;
...
export interface ConsumptionComparisonResponse {
...
const AdminReports: React.FC = () => {
  ...
  const fetchVehicleReport = async () => {
    try {
      ...
      const response = await api<any>(`/fuel/reports/vehicle?${params.toString()}`);
      console.log('Vehicle report received:', response);
      
      const transformedResponse = {
        reports: (response.reports || []).map((report: any) => ({
          vehicleId: report.vehicle_id || report.vehicleId,
          routeCount: report.route_count ?? report.routeCount ?? 0,
          totalEstimatedConsumption: report.total_estimated_consumption ?? report.totalEstimatedConsumption ?? 0,
          totalRealConsumption: report.total_real_consumption ?? report.totalRealConsumption ?? 0,
          totalDistanceKm: report.total_distance_km ?? report.totalDistanceKm ?? 0,
          averageConsumptionPerKm: report.average_consumption_per_km ?? report.averageConsumptionPerKm ?? 0,
      ...
      setVehicleReport(transformedResponse);
    } catch (err: any) {
      console.error('Error fetching vehicle report:', err);
      setError(err.message || 'Error al cargar el reporte');
      addToast('Error al cargar el reporte', 'error');
    } finally {
      setLoading(false);
  ...
  const fetchDriverReport = async () => {
    try {
      ...
      const response = await api<any>(`/fuel/reports/driver?${params.toString()}`);
      console.log('Driver report received:', response);
      
      const transformedResponse = {
        reports: (response.reports || []).map((report: any) => ({
          driverId: report.driver_id || report.driverId,
          routeCount: report.route_count ?? report.routeCount ?? 0,
          totalEstimatedConsumption: report.total_estimated_consumption ?? report.totalEstimatedConsumption ?? 0,
          totalRealConsumption: report.total_real_consumption ?? report.totalRealConsumption ?? 0,
          totalDistanceKm: report.total_distance_km ?? report.totalDistanceKm ?? 0,
      ...
      setDriverReport(transformedResponse);
    } catch (err: any) {
  ...
  const fetchRouteReport = async () => {
    try {
      ...
      const response = await api<any>(`/fuel/reports/route?${params.toString()}`);
      console.log('Route report received:', response);
      
      const transformedResponse = {
        reports: (response.reports || []).map((report: any) => ({
          routeId: report.route_id || report.routeId,
          vehicleId: report.vehicle_id || report.vehicleId || '',
          driverId: report.driver_id || report.driverId || '',
          machineryType: report.machinery_type ?? report.machineryType ?? 0,
          estimatedConsumption: report.estimated_consumption ?? report.estimatedConsumption ?? 0,
          realConsumption: report.real_consumption ?? report.realConsumption ?? 0,
          distanceKm: report.distance_km ?? report.distanceKm ?? 0,
          differenceLiters: report.difference_liters ?? report.differenceLiters ?? 0,
          differencePercentage: report.difference_percentage ?? report.differencePercentage ?? 0,
      ...
      setRouteReport(transformedResponse);
    } catch (err: any) {
      console.error('Error fetching route report:', err);
      setError(err.message || 'Error al cargar el reporte');
      addToast('Error al cargar el reporte', 'error');
    } finally {
      setLoading(false);
              ...
              return (
                ...
                <div className="fuel-card p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-semibold text-white">Reportes por Tipo de Maquinaria</h2>
                    <button
                      onClick={() => {
                        try {
                        ...
                        <div key={index} className="border border-slate-700 rounded-lg p-4 bg-slate-800/30">
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <span className="px-3 py-1 bg-blue-500/20 text-blue-400 text-sm rounded-full border border-blue-500/30">
                              {report.registerCount} {report.registerCount === 1 ? 'registro' : 'registros'}
                            </span>
                          </div>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="p-3 bg-slate-800/50 rounded-lg">
                              <div className="text-xs text-slate-400 mb-1">Estimado</div>
                            ...
                            <div className="p-3 bg-slate-800/50 rounded-lg">
                              ...
                              <div className={`text-lg font-semibold ${getDifferenceColor(report.differencePercentage)}`}>
                                {formatNumber(report.differenceLiters)} L
                              </div>
                              <div className={`text-xs ${getDifferenceColor(report.differencePercentage)}`}>
                                ({formatNumber(report.differencePercentage)}%)
                              </div>
                            </div>
                            <div className="p-3 bg-slate-800/50 rounded-lg">
                              <div className="text-xs text-slate-400 mb-1">Promedio/km</div>
                              <div className="text-lg font-semibold text-purple-400">
                                {formatNumber(report.averageConsumptionPerKm, 3)} L/km
                              </div>
                              <div className="text-xs text-slate-400 mt-1">
                                {formatNumber(report.totalDistanceKm)} km total
                              </div>
                            </div>
                          </div>
                        </div>
